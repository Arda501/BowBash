...
/**
 * A block-for-block, contents-and-all snapshot of an arena's whole bounding box (its two
 * configured corners), explicitly captured by an admin ({@code /bb savemap}) and persisted to
 * disk so it survives a restart - restored exactly at the end of every round from then on,
 * covering everything that changed, broken or placed, including chest/barrel/furnace/etc.
 * contents (plain {@link BlockData} alone doesn't cover what's inside a container - only its
 * orientation/type/waterlogged/etc.). Mirrors the save-the-structure/reset-to-baseline approach
 * from the user's fabric-example-mod-26.2 gamemode (and its own Recored Paper port).
 */
public class MapSnapshot {
	...
	/** Captures every block between {@code pos1} and {@code pos2} (inclusive, any corner order) - and any container's contents. */
